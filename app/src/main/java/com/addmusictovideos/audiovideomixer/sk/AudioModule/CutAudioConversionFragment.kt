...
import com.addmusictovideos.audiovideomixer.sk.databinding.FragmentCutAudioConversionBinding
...
class CutAudioConversionFragment : Fragment() {
    private lateinit var binding: FragmentCutAudioConversionBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = FragmentCutAudioConversionBinding.inflate(layoutInflater, container, false)
        return binding.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        val args = CutAudioConversionFragmentArgs.fromBundle(requireArguments())
    ...
    private fun trimAudio() {
        outputPath = if (Build.VERSION.SDK_INT == Build.VERSION_CODES.Q) {
            ...
            File(requireContext().getExternalFilesDir(Environment.DIRECTORY_MUSIC),
                "cut_audio_${System.currentTimeMillis()}.mp3").absolutePath
        } else {
            // Other Android versions (Public directory)
            FilePathManager(requireContext()).getTrimAudioParentDir() +
                    "/cut_audio_${System.currentTimeMillis()}.mp3"
        }
                ...
                activity?.runOnUiThread {
                    context?.let { ctx ->
                        if (ReturnCode.isSuccess(returnCode)) {
                            ...
                            val action = CutAudioConversionFragmentDirections.actionCutaudioconversionToNavFinaloutput(outputPath,outputPath)
